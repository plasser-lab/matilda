...
After launching the script, the user is prompted to select the type of spectrum (absorption or emission) and input several parameters.
It interactively guides the user through parameter selection, and produces numerical data and graphs.
...
Modes, whose wave number is above w_min and whose Huang-Rhys factor is above S_min, are chosen for the Franck-Condon treatment.
Any other modes are treated via semi-classical broadening.
